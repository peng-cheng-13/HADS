...
void FileSystem::parseMessage(char *bufferRequest, char *bufferResponse) 
{
    ...
    switch(bufferGeneralSend->message) {
	    ...
	    //uint32_t tier = 0; /*To be implemented: Intelligent data palcement*/
            //bufferGeneralReceive->result = moveAndUpdateMeta(bufferSend->path, tier);
...
bool FileSystem::fillRDMARegion(uint64_t size, uint64_t offset, file_pos_info *fpi, FileMeta *metaFile) {
	...
	if (storage->tableBlock->create(&indexCurrentExtraBlock) == false) {
	    ...
	    Debug::debugItem("Init RDMA block, id = %d", BlockID);
	    ...
	    /*Copy data*/
	    if(metaFile->tier == 0) {
	        void *dest = (void *)(metaFile->tuple[BlockID].StorageAddress);
	        void *src  = (void *)(RdmaZoneBaseAddress + indexCurrentExtraBlock * BLOCK_SIZE);
    ...
    if (fpi->len == 1) {
        ...
        fpi->tuple[fpi->len - 1].size = (offset + size) % BLOCK_SIZE;
    }
 ...
 * @param   NewmetaFile New Metadata of the file.
 * @return              If operation succeeds then return true, otherwise return false.
 */
bool FileSystem::moveAndUpdateMeta(FileMeta *metaFile) {
    uint32_t tier;
    FileMeta *NewmetaFile;
    Debug::debugTitle("FileSystem::copyAndPrepareMeta");
    int StorageTier = (int) tier;
    int i,j,k;
    /*Init memory address*/
    uint64_t MemZoneBaseAddress = server->getMemoryManagerInstance()->getExtraDataAddress();
    uint64_t RdmaZoneBaseAddress = server->getMemoryManagerInstance()->getDataAddress();
    Debug::debugItem("Stage 1-1. Init memory address");
    
    if (StorageTier == 0) { /*Copy data from memory tier to RDMA Zone*/

	Debug::debugItem("Stage 2. Copy data from memory tier to RDMA Zone");
	uint64_t countExtraBlock; /* Count of extra blocks. At least 1. */
        int64_t boundCurrentExtraExtent; /* Bound of current extra extent. */
        boundCurrentExtraExtent = -1; /* Bound current extra extent start from 0. */
        countExtraBlock = metaFile->size / BLOCK_SIZE + 1;
	Debug::debugItem("Stage 2-1, countExtraBlock = %d, BLOCK_SIZE = %d", countExtraBlock, BLOCK_SIZE);
	uint64_t indexCurrentExtraBlock; /* Index of current extra block. */
        uint64_t indexLastCreatedBlock;
	bool resultFor = true;
        k = 0;
	...
	for (i = 0; i < (int) metaFile->count; i++) {
		...
		Debug::debugItem("Stage 3-1, Update block: %d", k);
		if (storage->tableBlock->create(&indexCurrentExtraBlock) == false) {
                    resultFor = false; /* Fail due to no enough space. Might cause inconsistency. */
                    break;
                } else { /* So we need to modify the allocation way in table class. */
		    indexLastCreatedBlock = -1;
		    if(boundCurrentExtraExtent >= 0) {
                        indexLastCreatedBlock = NewmetaFile->tuple[boundCurrentExtraExtent].indexExtentStartBlock
                            + NewmetaFile->tuple[boundCurrentExtraExtent].countExtentBlock;
                    }
		    Debug::debugItem("boundCurrentExtraExtent: %d, indexLastCreatedBlock: %d, indexCurrentExtraBlock: %d",
                        boundCurrentExtraExtent, indexLastCreatedBlock, indexCurrentExtraBlock);
		    if(boundCurrentExtraExtent >= 0 &&
                        (indexCurrentExtraBlock == indexLastCreatedBlock))
                    {
                        NewmetaFile->tuple[boundCurrentExtraExtent].countExtentBlock++; /* Increment of count of blocks in current extent. */
                    } else if (boundCurrentExtraExtent == -1 ||
                        indexCurrentExtraBlock != indexLastCreatedBlock) { /* Separate block index generates a new extent. */
                        boundCurrentExtraExtent++;
                        NewmetaFile->tuple[boundCurrentExtraExtent].hashNode = hashLocalNode; /* Assign local node hash. */
                        NewmetaFile->tuple[boundCurrentExtraExtent].indexExtentStartBlock = indexCurrentExtraBlock; /* Assign start block in extent. */
                        NewmetaFile->tuple[boundCurrentExtraExtent].countExtentBlock = 1; /* Assign count of block in extent. */
                        NewmetaFile->count++; /* Update count of extents. */
                    } else{ /* Continuous blocks in one extent. */
                        resultFor = false;
                        break;
                    }
		...
		k++;
		Debug::debugItem("Stage 4, Ready to copy block data");
		void *dest = (void *)(RdmaZoneBaseAddress + (NewmetaFile->tuple[boundCurrentExtraExtent].indexExtentStartBlock +
                    NewmetaFile->tuple[boundCurrentExtraExtent].countExtentBlock - 1) * BLOCK_SIZE);
		void *src  = (void *)(MemZoneBaseAddress + (metaFile->tuple[i].indexExtentStartBlock + j) * BLOCK_SIZE);
		memcpy(dest, src, BLOCK_SIZE);
		//Debug::debugItem("Stage 4-1, copy block data once, src = %ld, dest = %ld", (long)*dest, (long)*src);

	    } /*End for loop (j)*/
	} /*End for loop (i)*/
    } /*End memory storage tier*/
}
    ...
    if (path == NULL) { /* Judge if path and file meta buffer are valid. */
...

	    {
			...
			if (storage->tableFileMeta->get(indexFileMeta, &metaFile) == false) {
			    ...
			    /*Allocate a new FileMeta*/
			    FileMeta *NewmetaFile = (FileMeta *)malloc(sizeof(FileMeta));
			    /*Initilize the NewmetaFile*/
			    uint64_t MemZoneBaseAddress = server->getMemoryManagerInstance()->getExtraDataAddress();
			    uint64_t RdmaZoneBaseAddress = server->getMemoryManagerInstance()->getDataAddress();
			    if (tier == 0) { /*Move data to the memory tier*/

				/*Update FileMetaTuple*/
				Debug::debugItem("Stage 4. Move data to the memory tier");
				uint64_t countExtraBlock; /* Count of extra blocks. At least 1. */
                                int64_t boundCurrentExtraExtent; /* Bound of current extra extent. */
				boundCurrentExtraExtent = -1; /* Bound current extra extent start from 0. */
                                countExtraBlock = metaFile.size / BLOCK_SIZE + 1;
				...
				for (i = 0; i < (int) metaFile.count; i++) {
				    for(j = 0; j < (int) metaFile.tuple[i].countExtentBlock; j++) {
					Debug::debugItem("Stage 5-1, Update block: %d", k);
					if (storage->extraTableBlock->create(&indexCurrentExtraBlock) == false) {
                                            resultFor = false; /* Fail due to no enough space. Might cause inconsistency. */
                                            break;
                                        } else { /* So we need to modify the allocation way in table class. */
					    indexLastCreatedBlock = -1;
					...
					k++;
					Debug::debugItem("Stage 6. Ready to copy block");
					void *dest = (void *)(MemZoneBaseAddress + (NewmetaFile->tuple[boundCurrentExtraExtent].indexExtentStartBlock +
					    NewmetaFile->tuple[boundCurrentExtraExtent].countExtentBlock - 1) * BLOCK_SIZE);
					void *src  = (void *)(RdmaZoneBaseAddress + (metaFile.tuple[i].indexExtentStartBlock + j) * BLOCK_SIZE);
					memcpy(dest, src, BLOCK_SIZE);
					Debug::debugItem("Stage 6-1. Copy block data once");
					/*Remove old block*/
					storage->tableBlock->remove(metaFile.tuple[i].indexExtentStartBlock + j);
					Debug::debugItem("Stage 6-2. Remove RDMA block data");
				    } /*End for loop (j)*/
				} /*End for loop (i)*/
				...
				if(resultFor == false) {
				    ...
				    NewmetaFile->size = metaFile.size;
				    Debug::debugItem("Newmeta.size: %ld", (long) NewmetaFile->size);
				    result = true;
				}
			    ...
			    if(result) {
		                NewmetaFile->timeLastModified = time(NULL);
		                //storage->tableFileMeta->remove(indexFileMeta);
		                storage->tableFileMeta->put(indexFileMeta, NewmetaFile);
		            }
		            free(NewmetaFile);
		            unlockWriteHashItem(key, hashNode, hashAddress); /*Unlock hash item*/
			...
			if (storage->tableFileMeta->get(indexFileMeta, metaFile) == false) {
				...
				if ((metaFile->size == 0) || ((offset + size - 1) / BLOCK_SIZE > (metaFile->size - 1) / BLOCK_SIZE)) { /* Judge if new blocks need to be created. */
					...
					for (uint64_t i = 0; i < countExtraBlock; i++) {
					    Debug::debugItem("for loop, i = %d, BlockID = %d", i, (int)BlockID + 1);
					...
					Debug::debugItem("Stage 4. Write to SSD tier");
					metaFile->tier = 0;
				    } /*End storage tier*/

				    metaFile->size = offset + size;
                                    Debug::debugItem("Stage end, meta.size: %ld", (long)metaFile->size);
                                    fillFilePositionInformation(size, offset, fpi, metaFile); /* Fill file position information. */
                                    result = true;
				} else {
				    /*Fill RDMA Zone*/
				    ...
				    fillRDMARegion(size, offset, fpi, metaFile);
				    metaFile->isNewFile = true;
				    result = true;
			    ...
			    Debug::debugItem("Stage 8. meta.size = %ld",(long) metaFile->size);
	                    free(metaFile);
			    Debug::debugItem("Stage end.");
			    return result;
